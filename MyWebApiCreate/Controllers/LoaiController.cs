...
namespace MyWebApiCreate.Controllers
{
    ...
    public class LoaiController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public IActionResult Update(int id, LoaiVM _loai )
        {
            try
            {
                if (id == _loai.MaLoai)
                {
                    _loaiRepository.Update(_loai);
                    return NoContent();
                }
                return NotFound();
        ...
        [HttpPost]
        public IActionResult Add(LoaiModel _loai)
        {
            try
            {
                return Ok(_loaiRepository.Add(_loai));
            }
            catch
            {
                return StatusCode(StatusCodes.Status500InternalServerError);
